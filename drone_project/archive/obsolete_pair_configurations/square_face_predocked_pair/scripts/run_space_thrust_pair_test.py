#!/usr/bin/env python3
r"""One-command launcher for the two-drone Space-thrust test.

Normal use:

    python configurations\square_face_predocked_pair\scripts\run_space_thrust_pair_test.py
"""
...
SPACE_TEST = CONFIG_ROOT / "controller" / "space_thrust_pair_test.py"


def parse_args() -> tuple[argparse.Namespace, list[str]]:
    parser = argparse.ArgumentParser(
        description="Generate the dynamic pre-docked pair scene and run the Space-thrust test."
    )
    ...
    parser.add_argument("--gap", type=float, default=None, help="Face-to-face clearance [m].")
...
def main() -> int:
    ...
    if not args.skip_generate:
        ...
        print("Generating dynamic two-drone pair scene...", flush=True)
        subprocess.run(generator_command, cwd=PROJECT_ROOT, check=True)

    controller_command = add_common_args(
        [sys.executable, str(SPACE_TEST), "--load-scene"],
        args,
    )
    controller_command.extend(controller_args)
    print("Starting two-drone Space-thrust controller...", flush=True)
