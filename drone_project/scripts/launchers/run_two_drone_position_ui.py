#!/usr/bin/env python3
"""One-command launcher: spawn drones, then open the multi-drone docking UI."""
...
def parse_args() -> tuple[argparse.Namespace, list[str]]:
    parser = argparse.ArgumentParser(description="Spawn drones and run the multi-drone docking UI.")
...
def main() -> int:
    ...
    run_step(spawn_command, "Spawning separated drone plant models...", check=True)

    controller_command = extend_connection_args(python_command(CONTROLLER), args)
    controller_command.extend(controller_args)
    return run_step(controller_command, "Starting multi-drone docking UI controller...")
