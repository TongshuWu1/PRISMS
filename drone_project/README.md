...
This is the original PRISMS drone project, now organized under
`drone_project/` inside the PRISMS workspace.
...
Docked assemblies are commanded as a six-DOF target: `x`, `y`, `z`, roll, pitch, and yaw. Roll and pitch are interpreted as offsets from the attitude stored when the assembly is latched or reset, so a tilted hex-to-hex pair has zero roll/pitch at its intended docked attitude rather than at a level upright attitude. The docked defaults now use stronger assembly-specific acceleration and torque limits than the first cautious baseline; these can be tuned with `--assembly-max-horizontal-accel`, `--assembly-max-vertical-accel`, `--assembly-attitude-torque-gain-*`, and `--assembly-torque-limit-*`.

For underactuated directions, the docked controller couples translation demand into attitude. The default `--assembly-attitude-coupling force-align` mode computes the assembly collective thrust axis in the leader frame and rotates the desired docked attitude toward the requested total force vector. This matters for the symmetric hex-pair case: a direct lateral force in the face-adjacent direction may be weak at the stored reference attitude, so the controller must tilt the assembly to create that acceleration. Tune this with `--assembly-force-alignment-gain` and `--assembly-force-alignment-limit-deg`; use `--assembly-attitude-coupling feedforward` only to compare against the older roll/pitch heuristic.

Yaw control is also tilted-assembly aware. With `--assembly-world-yaw-attitude` enabled by default, the attitude controller decomposes attitude error into world-horizontal tilt error and world-vertical yaw error. This prevents a yaw command from being treated as pure torque about the tilted leader module's body `+Z` axis, which was unstable for the hex pair. Docked yaw is intentionally protected with a slower command trajectory and a yaw-error clamp because fast yaw creates large internal loads at the dock. In the UI, tune this live with the `Dock yaw speed`, `Dock yaw accel`, and `Yaw error clamp` sliders.

When the docked controller is active, the UI also draws an assembly intent vector from the assembly center of mass. By default this is amber and shows `wrench_cmd[:3] / mass - g * ez`, the controller-requested COM acceleration after removing hover gravity. That is the clearest view of what the controller is trying to do. Use `--assembly-force-arrow-source achieved-accel` for the allocated acceleration after motor limits, `--assembly-force-arrow-source residual-accel` for the missing acceleration, or `--assembly-force-arrow-source achieved` for raw motor force including hover thrust.
