...
- The reference generator is guarded by a cable-aware speed governor; it slows down when tracking error, thrust use, allocation residual, contact validity, or poor cable geometry indicates the path is becoming dynamically risky. This is important because cable-efficient motion must wait for the reel and suspended dynamics instead of forcing the drones to chase an aggressive reference.
...
- Tilt regulation is scheduled. During transport, the drones do not try to keep the payload level; they only damp angular rate and reject enough cable torque to avoid uncontrolled spin. After the tool reaches the commanded spot and the estimated velocity is small, the hold controller regulates the stable cable-supported equilibrium tilt, not zero tilt.
...
- Hold allocation uses a geometry-dependent force/torque balance: good cable geometry prioritizes the cable-supported equilibrium attitude tightly, while shallow cable geometry relaxes the torque residual scale so the saturated tilted drones settle instead of entering a slow hover oscillation.
...
- `CONTROLLER_NOTES.md` records the controller architecture, research basis,
  benchmark takeaway, and the next constrained-MPC direction.
